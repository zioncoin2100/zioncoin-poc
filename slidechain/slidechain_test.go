...
func TestImport(t *testing.T) {
	...
	for _, tt := range importtests {
		...
		zioncoinAsset := makeAsset(tt.assetType, tt.code, tt.issuer)
		assetXDR, err := zioncoinAsset.MarshalBinary()
		if err != nil {
			t.Fatal(err)
		}
		...
		withTestServer(ctx, t, func(ctx context.Context, db *sql.DB, s *submitter, server *httptest.Server, chain *protocol.Chain) {
			...
			_, err = db.Exec("INSERT INTO pegs (nonce_hash, amount, asset_xdr, recipient_pubkey, nonce_expms, zioncoin_tx) VALUES ($1, 1, $2, $3, $4, 1)", nonceHash[:], assetXDR, testRecipPubKey, expMS)
			if err != nil {
				t.Fatal(err)
			}
			c.imports.Broadcast()
...
func TestEndToEnd(t *testing.T) {
	...
	withTestServer(ctx, t, func(ctx context.Context, db *sql.DB, s *submitter, sv *httptest.Server, ch *protocol.Chain) {
		hclient := &equator.Client{
			URL:  "https://equator-testnet.zion.info",
			HTTP: new(http.Client),
		}
		root, err := hclient.Root()
		if err != nil {
			t.Fatalf("error getting equator client root: %s", err)
		}
		...
		err = zioncoin.FundAccount(exporter.Address())
		if err != nil {
		...
		for _, tt := range tests {
			// Prepare Zioncoin account to peg-in funds and txvm account to receive funds.
			...
			pegInTx, err := zioncoin.BuildPegInTx(exporter.Address(), uniqueNonceHash, inputAmount.HorizonString(), "", "", c.AccountID.Address(), hclient)
			if err != nil {
				t.Fatalf("error building peg-in tx: %s", err)
			}
			succ, err := zioncoin.SignAndSubmitTx(hclient, pegInTx, exporter.Seed())
			if err != nil {
			...
			go func() {
				...
				err := c.hclient.StreamTransactions(ctx, exporter.Address(), &cur, func(tx equator.Transaction) {
