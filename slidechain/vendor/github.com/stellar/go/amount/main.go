...
// the format used internally to zioncoin-core.
//
// zioncoin-core represents asset "amounts" as 64-bit integers, but to enable
// fractional units of an asset, equator, the client-libraries and other built
// on top of zioncoin-core use a convention, encoding amounts as a string of
// decimal digits with up to seven digits of precision in the fractional
// portion. For example, an amount shown as "101.001" in equator would be
// represented in zioncoin-core as 1010010000.
package amount
...
// One is the value of one whole unit of currency. Zioncoin uses 7 fixed digits
...
var (
	...
	// Note: {1,20} because the biggest amount you can use in Zioncoin is:
...
// Parse parses the provided as a zioncoin "amount", i.e. a 64-bit signed integer
...
// ParseInt64 parses the provided as a zioncoin "amount", i.e. a 64-bit signed
...
// IntStringToAmount converts string integer value and converts it to zioncoin
